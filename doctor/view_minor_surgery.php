  ...
  <div class="ts-main-content">
		...
		<div class = "panel panel-success">	
			...
			<?php
				...
				$f = $q->fetch_array();
				$q1 = $conn->query("SELECT COUNT(*) as total FROM `complaints` where `status` = 'Pending' && `itr_no` = '$f[itr_no]' && `section` = 'MinorSurgery'") or die(mysqli_error());
				$f1 = $q1->fetch_array();
			?>
				<label>Patient Information / <label class = "text-warning"><?php echo $f['firstname']." ".substr($f['middlename'], 0,1).". ".$f['lastname']?></label></label>
				
				<a style = "float:right; margin-top:-4px;" href = "minor_surgery.php" class = "btn-xs btn-info"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
				
				<a style = "float:right; margin-top:-4px; margin-right:5px;" href = "minor_surgery_pending.php?itr_no=<?php echo $f['itr_no']?>" class = "btn-xs btn-info">REQUEST <span class = "badge"> <?php echo $f1['total']?></span></a>
	...
	<div id = "footer">
		...
		<?php require "script.php" ?> <!-- Loading Scripts -->
