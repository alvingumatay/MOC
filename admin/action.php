...
if(isset($_POST["action"]))
{
		...
		foreach ($result as $row) 
		{
		  ...
		  $output .='
             <tr>
                ...
                <td>'.$status.'</td>
                <td>
                    <button type="button" name="action" class="btn btn-info btn-xs action" data-itr_no="'.$row["servtype"].'" data-status="'.$row["status"].'">Accept</button>
                </td>
            </tr>
		  ';
		}
	...
	if ($_POST["action"] == 'change_status') {
		...
		$query =  'UPDATE appointment  SET status = :status WHERE  servtype=:servtype';
